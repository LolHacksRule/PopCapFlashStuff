package com.popcap.flash.bejeweledblitz.game.ui.raregems.phoenixprism
{
   ...
   public class Phoenix extends Sprite
   {
      ...
      public function Phoenix(param1:Blitz3App)
      {
         super();
         this.m_App = param1;
      ...
      {
         var _loc1_:Number = this.m_Radius;
         this.m_Sway1 = Math.sin(_loc1_ * 0.05);
         this.m_Fire.graphics.clear();
         var _loc2_:Number = -Math.PI;
         while(_loc2_ < Math.PI)
         {
            this.m_Sway2 = Math.sin(_loc2_ * 4 + this.m_Sway1);
            this.m_MoveX = _loc1_ * Math.sin(_loc2_) + _loc1_ * this.m_Sway1;
            this.m_MoveY = 2 * (-_loc1_ * Math.abs(Math.cos(_loc2_)) - _loc1_ * Math.abs(this.m_Sway2));
            this.m_ControlX = _loc1_ * Math.sin(_loc2_) + _loc1_ * Math.cos(_loc2_);
            this.m_ControlY = 0;
            this.m_Fire.graphics.lineStyle(1,COLORS[Math.floor(6 - Math.abs(_loc2_ * 2))],0.2 * Math.random());
            this.m_Fire.graphics.beginFill(FIRES[Math.floor(Math.abs(_loc2_ * 2))],0.1 + 0.1 * Math.random());
