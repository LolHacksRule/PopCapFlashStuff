package com.popcap.flash.bejeweledblitz.messages.newuser
{
   ...
   public class FriendscoreMessage extends MessageObject
   {
       
      
      private const TRACKING_ID:String = "NewUserMessage/Tournament";
      
      private const TRACKING_ID_CANCEL:String = "NewUserMessage/Tournament/Continue";
      
      private const TRACKING_ID_SHARE:String = "NewUserMessage/Tournament/InviteFriends";
      
      public function FriendscoreMessage(app:Blitz3App)
      {
         super(app);
         m_Header = new MessageHeader(m_App,NewUserMessagesLoc.LOC_MSG_TOURNAMENT_HEADER);
         m_Header.y = 74;
         addChild(m_Header);
         ...
         m_Icon.y = m_Header.y + m_Header.height + 10;
         m_Icon.filters = [new GlowFilter(16777113,1,4,4,2)];
         addChild(m_Icon);
         m_MessageText = m_App.TextManager.GetLocString(NewUserMessagesLoc.LOC_MSG_TOURNAMENT_TEXT);
         m_TextVerticalPos = m_Icon.y + m_Icon.height + 15;
         m_TrackingId = this.TRACKING_ID;
         m_Buttons = new ButtonWidgetDouble(m_App);
         m_Buttons.SetLabels(m_App.TextManager.GetLocString(NewUserMessagesLoc.LOC_BTN_LABEL_INVITE),m_App.TextManager.GetLocString(NewUserMessagesLoc.LOC_BTN_LABEL_CANCEL));
