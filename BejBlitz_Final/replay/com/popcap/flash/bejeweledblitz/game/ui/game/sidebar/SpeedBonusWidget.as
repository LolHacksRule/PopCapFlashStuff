package com.popcap.flash.bejeweledblitz.game.ui.game.sidebar
{
   ...
   public class SpeedBonusWidget extends Sprite
   {
      ...
      {
         var levelFormat:TextFormat = null;
         levelFormat = new TextFormat();
         levelFormat.font = Blitz3GameFonts.FONT_BLITZ_STANDARD;
         levelFormat.size = 14;
         ...
         this.mNormalLevelText.htmlText = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_GAMEPLAY_BONUS_SPEED) + 1000;
         ...
         bonusFormat.font = Blitz3GameFonts.FONT_BLITZ_STANDARD;
         bonusFormat.size = 14;
         ...
         this.mNormalTextSprite.x = -this.mNormalLevelText.width * 0.5;
         this.mNormalTextSprite.y = -this.mNormalLevelText.height * 0.5;
         this.mBlazeTextSprite = new Sprite();
         this.mBlazeTextSprite.addChild(this.mBlazeLevelText);
         this.mBlazeTextSprite.x = -this.mBlazeLevelText.width * 0.5;
         this.mBlazeTextSprite.y = -this.mBlazeLevelText.height * 0.5;
         ...
         this.mNormalBulgeSprite.x = this.mNormalTextSprite.width * 0.5;
         this.mNormalBulgeSprite.y = this.mNormalTextSprite.height * 0.5;
         this.mBlazeBulgeSprite = new Sprite();
         this.mBlazeBulgeSprite.addChild(this.mBlazeTextSprite);
         this.mBlazeBulgeSprite.x = this.mBlazeTextSprite.width * 0.5;
         this.mBlazeBulgeSprite.y = this.mBlazeTextSprite.height * 0.5;
      ...
      public function Update() : void
      {
         ...
         if(this.m_App.logic.speedBonus.GetLevel() > this.mLastLevel)
         {
            this.mLastLevel = this.m_App.logic.speedBonus.GetLevel();
            this.mBulgeTimer = BULGE_TIME;
         }
         if(this.m_App.logic.speedBonus.GetLevel() < this.mLastLevel && this.mLastLevel > 0)
         {
            --this.mFadeTimer;
         ...
         this.mNormalLevelText.htmlText = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_GAMEPLAY_BONUS_SPEED) + this.m_App.logic.speedBonus.GetBonus();
         this.mBlazeLevelText.htmlText = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_GAMEPLAY_BONUS_SPEED) + this.m_App.logic.speedBonus.GetBonus();
      }
      ...
      public function Draw() : void
      {
         ...
         this.mNormalLevelText.textColor = 16777215;
         this.mNormalBonusText.textColor = 16777215;
         var blazeMin:Number = this.m_App.logic.blazingSpeedLogic.GetPercent();
         var blazePercent:Number = Math.max(0,(blazeMin - 0.5) / (1 - 0.5));
         this.mScrollRect.width = 104 * blazePercent;
         this.mBlazeTextSprite.scrollRect = this.mScrollRect;
         this.mBlazeLevelText.textColor = 16742144;
         this.mBlazeBonusText.textColor = 16742144;
      }
   }
}
