package com.popcap.flash.bejeweledblitz.game.ui.gameover.levels
{
   ...
   public class LevelView extends Sprite
   {
      ...
      public function LevelView(app:Blitz3App)
      {
         ...
         this.m_LevelCrest = new LevelCrest(app);
      ...
      public function SetData(xp:Number) : void
      {
         ...
         var xpRemaining:String = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_LEVEL_BAR_XP_REMAINING);
         xpRemaining = xpRemaining.replace("%s",StringUtils.InsertNumberCommas(Math.ceil(nextLevelCutoff - xp)));
         if(this.m_IsDisabled)
         {
            xpRemaining = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_LEVEL_BAR_XP_REMAINING_DISABLED);
            levelName = this.m_App.TextManager.GetLocString(Blitz3GameLoc.LOC_LEVEL_NAME_DISABLED);
         }
